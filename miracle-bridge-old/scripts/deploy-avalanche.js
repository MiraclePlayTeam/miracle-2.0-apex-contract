...
async function main() {
    ...
    console.log(`MPN Token address: ${mpnToken.address}`);

    console.log(
        `MPT Token address: ${mptToken.address} /
        MLGE Token address: ${mlgeToken.address} /
        MPN Token address: ${mpnToken.address} /
    ...
    const bridgeProxyContract = await deployContract(Proxy, owner.address, { nonce: 13 });
    console.log("Proxy contracts deployed end!");

    console.log(`MPN Token Proxy address: ${mpnTokenProxyContract.address}`);

    console.log(
        `MPT Token Proxy address: ${mptTokenProxyContract.address}
        / MLGE Token Proxy address: ${mlgeTokenProxyContract.address}
        / MPN Token Proxy address: ${mpnTokenProxyContract.address}
    ...
    await mlgeTokenProxyContract.upgrade("0x52c6ff68da63dae7ea7fc40e768e8ffec830adce");
    await mpnTokenProxyContract.upgrade(mpnToken.address);
    await bridgeProxyContract.upgrade("0xa917761Bb88a04f828D60bB9Cc00A44dbBfF4D62");
    console.log("Proxy contracts upgrade end!");

    // Initialize proxy contracts
    mptTokenProxy = await getContractAt("TokenERC20", "0x5321C14A124D33dA74A4701dCFBcB13e7C1fA707");
    mlgeTokenProxy = await getContractAt("TokenERC20", "0x8Df8d90c0E0f6bacC18bAB3FDe026374051E8635");
    mpnTokenProxy = await getContractAt("DropERC1155", mpnTokenProxyContract.address);
    bridgeProxy = await getContractAt("Bridge", "0x215e1bf3dd7ef0a5d5a740e965a3e090ca3b91e4");
    console.log("Proxy contracts initialize start!");
    await bridgeProxy.initialize("Bridge", owner.address, ChainType.Avalanche, owner.address, owner.address);
