...
// File @openzeppelin/contracts/governance/TimelockController.sol@v4.9.6
...
// File contracts/multisig/TimelockMultiSig.sol
...
contract TimelockMultiSig is AccessControl {
    ...
    constructor(address[] memory _owners, uint256 _required, address payable _timelock) {
        ...
        _setupRole(MULTISIG_ADMIN_ROLE, address(this));

        for (uint256 i = 0; i < _owners.length; i++) {
            address owner = _owners[i];
            require(owner != address(0), "invalid owner");

            _setupRole(SUBMIT_ROLE, owner);
        }

        required = _required;
        timelock = TimelockController(_timelock);
        _setupRole(EXECUTOR_ROLE, _timelock);
    }
