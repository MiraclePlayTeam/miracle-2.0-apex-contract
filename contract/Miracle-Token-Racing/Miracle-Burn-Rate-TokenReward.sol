...
contract BurnRaceProRata is PermissionsEnumerable, Multicall, ContractMetadata {
    ...
    IERC20 public rewardToken; // 리워드로 지급될 토큰
    ...
    struct Round {
        ...
        IERC20 rewardToken; // 라운드별 리워드 토큰
    }
    ...
    constructor(address _Token, address _admin, string memory _contractURI, address _rewardToken) {
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        if (_rewardTokenAddress == address(0)) {
            ...
            require(rewardToken.transferFrom(msg.sender, address(this), _totalReward), "Token transfer failed");
        }
    ...
    function claimReward(uint256 _roundId) external {
        ...
        if (rewardAmount > 0) {
            if (address(round.rewardToken) == address(0)) {
                (bool success,) = payable(msg.sender).call{value: rewardAmount}("");
                require(success, "MPT transfer failed");
            } else {
                require(round.rewardToken.transfer(msg.sender, rewardAmount), "Token transfer failed");
            }
        }
